    ...
    switch (Handle_Pick.state)
    {
		...
		case CYLINDER_GO_DOWN:
		{
			if (Handle_Pick.vacum_id == 1)
				Cylinder1_Go_Down;
			else if(Handle_Pick.vacum_id == 2)
				Cylinder2_Go_Down;
			delay_us(100);
...
{
    switch (Handle_Release.state)
    {
		case PICK_IDLE:
		{
			Handle_Release.retry = 0;
			Handle_Release.vacum_id  = vacum_id;
			Handle_Release.state = CYLINDER_GO_DOWN;
			break;
		}
		case CYLINDER_GO_DOWN:
		{
			if (Handle_Release.vacum_id == 1)
				Cylinder1_Go_Down;
			else if(Handle_Release.vacum_id == 2)
				Cylinder2_Go_Down;
			delay_us(100);
			Handle_Release.state = VACUM_REALSE_OFF;
			break;
		}
		case VACUM_REALSE_OFF:
		{
			if (Handle_Release.vacum_id == 1)
				...
				Vacum2_Release_Off;
			delay_us(50);
			Handle_Release.state = VACUM_PICK_ON;
			break;
		}
		case VACUM_PICK_ON:
		{
			if (Handle_Release.vacum_id == 1)
				...
				Vacum2_Pick_On;
			delay_us(100);
			Handle_Release.state = CYLINDER_GO_UP;
			break;
		}
		case CYLINDER_GO_UP:
		{
			if (Handle_Release.vacum_id == 1)
				...
				Cylinder2_Go_Up;
			delay_us(100);
			Handle_Release.state = PICK_RETRY;
			break;
		}
		case PICK_RETRY:
		{
			...
			if (check)
			{
				Handle_Release.state = PICK_DONE_OK;
			}
			else
			{
				if (++Handle_Release.retry < 2)
					Handle_Release.state = CYLINDER_GO_DOWN;
				else
					Handle_Release.state = PICK_DONE_NG;
			}
			break;
		}
		case PICK_DONE_OK:
		{
			Handle_Release.state = PICK_IDLE;
			return OK;
			break;
		}
		case PICK_DONE_NG:
